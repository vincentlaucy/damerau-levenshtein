function DamerauLevenshtein(prices, damerau) {
  ...
  if (!prices) prices = {};
  var insert, remove, substitute, transpose, shift;
  ...
  function distance(down, across) {
    ...
    function overrideIfFx(action, overrideFx) {
    ...
    _insert = overrideIfFx(insert, (function(source1, source2, a) {
      if (shift && (source1.indexOf(a) > -1) && (source2.indexOf(a) >
          -1)) return shift;
      return insert(a, source1, source2);
    ...
    if (down === across) {
      ...
      down.forEach(function(d, i) {
        ...
        across.forEach(function(a, j) {
          ...
          else if (i === 0) ds[i][j] = ds[i][j - 1] + _insert(a);
          // Down -> empty across (j == 0) by deleting
            ...
            ds[i][j] = Math.min(
              ...
              ds[i - 1][j - 1] + (d === a ? 0 : _substitute(d, a))
            );
            ...
            if (damerau && i > 1 && j > 1 && down[i - 1] === a && d ===
              across[j - 1]) {
              ds[i][j] = Math.min(
                ds[i][j],
                ds[i - 2][j - 2] + (d === a ? 0 : transpose(d,
                  down[i - 1]))
              );
            };
          };
